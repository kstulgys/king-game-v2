export default function appReducer(state, action) {
	switch (action.type) {
		...
		case 'SELECT_GAME':
			const r = window.confirm(`Do you really want to play ${action.selectedGame.title}?`)
			if (r == true) {
				const updatedGame = { ...action.selectedGame, played: true }
				const restGames = action.player.games.filter(g => g.title !== action.selectedGame.title)
				const updatedGames = [...restGames, updatedGame].sort((a, b) => a.no - b.no)
				const updatedPlayer = {
					...action.player,
					games: updatedGames
				}
				const restPlayers = state.players.filter(p => p.no !== action.player.no)
				const players = [...restPlayers, updatedPlayer].sort((a, b) => a.no - b.no)

				const currentGame = state.games.find(game => game.title == action.selectedGame.title)
				return {
					...state,
					players,
					currentGame,
